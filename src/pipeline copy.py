...
def main():
    ...
    print("Total Valid samples:", len(valid_list))
    ...
    preds, targets, _ = eval_point(head, model, testloader, device, desc="Test (point)")
    lower, upper = apply_intervals(preds, q_hat_final)
    coverage = ((targets >= lower) & (targets <= upper)).float().mean().item()
    print(f"[TEST] Conformal coverage={coverage:.3f} (alpha={args.alpha}, q_hat={q_hat_final:.4f})")
