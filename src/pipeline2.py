...
import argparse
from utils import load_mos_txt, AudioLoader, Preprocessor, AudioMOSDataset, PredictionHead, create_logger
import datetime
...
def train_one_epoch(head, model, loader, optimizer, device, log_interval=10, log=print):
    head.train()
    total_loss = 0.0
    total_mse = 0.0
    n_batches = 0
    for step, (wave, mos) in enumerate(loader, start=1):
        ...
        if step % log_interval == 0:
            log(f"Train Step {step} | OrdinalLoss {loss.item():.4f} | MSE {mse:.4f} | Pred mean {pred_mos.mean():.3f}")
    return total_loss / max(1, n_batches), total_mse / max(1, n_batches)

@torch.no_grad()
def eval_point(head, model, loader, device, desc="Eval", log=print):
    head.eval()
    total_mse = 0.0
    n_batches = 0
    preds = []
...
def main():
    ...
    try:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            train_loss, train_mse = train_one_epoch(head, model, trainloader, optimizer, device, log=log)
            log(f"Epoch {epoch} Train | Loss {train_loss:.4f} | MSE {train_mse:.4f}")

            _, _, valid_mse = eval_point(head, model, validloader, device, desc="Validation (point)", log=log)
            q_hat = conformal_calibrate(head, model, calibloader, device, alpha=args.alpha, log=log)
            q_hat_final = q_hat
        ...
        preds, targets, _ = eval_point(head, model, testloader, device, desc="Test (point)", log=log)
